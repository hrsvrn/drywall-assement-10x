...
FIXED VERSION: Properly handles gradient flow through SAM.
...
# --------------------------
def get_sam_prediction_with_gradients(model, image, prompt, device):
    """
    Get SAM prediction while maintaining gradients.
    
    Args:
        model: GroundedSAM instance
        image: numpy array (H, W, 3)
        prompt: string
        device: torch device
        
    Returns:
        pred_mask: torch tensor (1, H, W) with gradients
    """
    ...
    # If no detections, return empty mask (no gradients needed)
    if len(detections.xyxy) == 0:
        empty_mask = torch.zeros((1, image.shape[0], image.shape[1]), device=device)
        return empty_mask
    ...
    # This is the key difference - we use predict_torch which maintains gradients
    ...
    # Combine all masks (union) - this maintains gradients
    # Use max pooling to combine masks
    combined_mask = masks.max(dim=0)[0]  # (1, H, W)
...
for epoch in range(EPOCHS):
    ...
    for batch in pbar:
        ...
        for i, (image, prompt) in enumerate(zip(images, prompts)):
            try:
                # Get prediction WITH GRADIENTS
                pred_mask = get_sam_prediction_with_gradients(model, image, prompt, device)
    ...
    # VALIDATION
    # --------------------------
    model.sam_predictor.model.eval()
